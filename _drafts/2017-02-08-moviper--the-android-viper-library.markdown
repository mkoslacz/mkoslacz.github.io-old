    ...
    override fun attachView(view: LoginContract.View?) {
        ...
        addSubscription(
                view
                        ...
                        ?.doOnNext { routing.goToProfileScreen(it) }
                        ?.doOnError {
                            view?.hideLoading()
                            view?.showError(it)
                        }
...
So let's put it to the separate `ObservableExtensions` file in the `util` package and replace the mentioned calls using this metod:
